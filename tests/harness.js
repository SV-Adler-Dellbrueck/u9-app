...
async function starten(opt = {}) {
  ...
  await ctx.route("**/*", async r => {
    ...
    /* v512/v513: Alles unter uebungen/ wird beim Oeffnen abgeglichen und legt an, was fehlt –
       in JEDER Pruefung, die eine Sitzung vortaeuscht. Das faelscht jede Zaehlung von
       Schreibzugriffen. Standardmaessig antworten diese Dateien deshalb wie eine fehlende
       Datei (den Fall kennt der Abgleich: still nichts tun). Wer sie braucht:
       starten({bibliothek:true}). Derselbe Zuschnitt wie die Cache-Ausnahme in sw.js. */
