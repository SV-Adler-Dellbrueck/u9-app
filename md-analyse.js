...
function anRenderMeilensteine(){
  ...
  box.innerHTML=ms.length?ms.slice(0,12).map(m=>`<div style="display:flex;gap:8px;align-items:center;background:#ecfdf5;color:#065f46;border:1px solid #6ee7b7;border-radius:var(--rl);padding:8px 10px;margin-bottom:6px">
    ...
    <div style="font-size:var(--s-text);color:var(--text)"><strong>${esc(m.name)}</strong>: ${esc(m.label)}${m.isTotal?` +${m.diff}% Niveau`:` von ${m.from} auf ${m.to}`} – heute loben! 🎉</div>
  </div>`).join(""):'<div style="font-size:var(--s-klein);color:var(--text3)">Noch nicht genug Verlaufsdaten (mind. 2 Bewertungen mit ~2 Monaten Abstand pro Spieler).</div>';
}
async function anLoadServer(){
  const fair=document.getElementById("an-fairness"),form=document.getElementById("an-form");
  if(!fair||!form)return;
  fair.innerHTML='<div style="font-size:var(--s-klein);color:var(--text3)">Lade...</div>';form.innerHTML="";
  try{
    const r=await fetch(`${SB_URL}/rest/v1/blitz_ratings?select=datum,spieler,kriterien,wertung`,{headers:sbAuthHeaders()});
    if(sbCheck401(r))return;
    if(!r.ok){fair.innerHTML='<div style="font-size:var(--s-klein);color:var(--text3)">Keine Verbindung</div>';return;}
    ...
    form.innerHTML=formRows.length?formRows.map(f=>{const st=Math.max(1,Math.min(3,Math.round(f.rAvg)));return `<div style="display:flex;justify-content:space-between;align-items:center;font-size:var(--s-text);padding:5px 0;border-bottom:1px solid var(--border)"><span>${esc(f.name)}</span><span>${"★".repeat(st)}<span style="color:var(--border)">${"★".repeat(3-st)}</span> ${f.arrow}</span></div>`;}).join(""):'<div style="font-size:var(--s-klein);color:var(--text3)">Noch keine Blitz-Bewertungen.</div>';
  }catch(e){fair.innerHTML='<div style="font-size:var(--s-klein);color:var(--text3)">Offline</div>';}
}
...
function showMilestoneHint(){
  ...
  bar.style.cssText="position:fixed;left:12px;right:12px;bottom:12px;z-index:8000;background:#065f46;color:#fff;border-radius:14px;padding:12px 14px;box-shadow:0 6px 24px rgba(0,0,0,.25);font-size:var(--s-text);display:flex;align-items:center;gap:10px";
  bar.innerHTML=`<span style="font-size:var(--s-teil)">💡</span><div style="flex:1">Tipp: <strong>${esc(m.name)}</strong> hat sich bei „${esc(m.label)}" stark verbessert – heute loben!</div><button onclick="this.parentElement.remove()" style="background:rgba(255,255,255,.2);border:none;color:#fff;border-radius:6px;padding:8px 10px;cursor:pointer;min-height:44px">OK</button>`;
