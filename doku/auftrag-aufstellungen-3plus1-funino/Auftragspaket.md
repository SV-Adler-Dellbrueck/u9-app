# Auftragspaket: Sieben Einheiten für 3+1, FUNiño und die Kombination
...
| `vorlagen-nachtrag-kombi.json` | 1 weitere Vorlage: L5-6 |
| (Drive) `Einheiten 3+1 und FUNiño`, `Einheit L5-6` | Die Einheiten in Trainersprache nach dem festen Schema |
...
**L5-6 braucht nur ein Jugendtor.** Ein Feld, auf einer Seite das Jugendtor, auf der anderen
zwei Minitore. Station 1 nutzt die Jugendtor-Hälfte, Station 2 die Minitor-Hälfte; Hauptteil 3
...
`bibliothek.json` plus Nachträge): **keine Fehler**. `_evNettoHinweis` meldet für keine der sieben
etwas, `_evStationenHinweis(v, 2)` ebenfalls nicht. Voraussetzung dafür war die Erweiterung von
`EI_ORDNUNGEN` (Schritt 2) – ohne sie weist die Prüfung alle sieben ab. Alle dreizehn Skizzen sind
...
3. **Vorlagen:** Die sieben Einträge aus `vorlagen-nachtrag.json` und
...
5. **v551:** Die sieben Namen als Zusatz-Einheiten benennen (`ZUSATZ` um `L4-6`, `L5-4`,
   `L6-3`, `L4-7`, `L5-5`, `L6-4`, `L5-6` erweitern). Die Prüfung der zwanzig
...
2. Der Abgleich legt 13 Übungen und 7 Vorlagen an und rührt den Bestand nicht an.
...
5. Keine der sieben löst einen Netto-Hinweis aus; alle tragen Skalierung 8/12/16 und eine
...
8. `L5-6` übernehmen bei zwei Feldtrainern: Hauptteil 1 und 2 wie Kriterium 3 mit den
   ...
   Ball“, Hauptteil 3 trägt „3+1 gegen FUNiño – großes Tor gegen zwei kleine“.

## Erst prüfen, nicht bauen

- **Hauptteil 3 auf einem Feld (3+1 und L5-6):** Bei 12 Kindern spielen beide Gruppen auf
