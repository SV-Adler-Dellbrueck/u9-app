# Auftragspaket: Neun Einheiten für 3+1, FUNiño und die Kombination
...
| `vorlagen-nachtrag-kombi.json` | 3 weitere Vorlagen: L5-6, L6-5 und L4-8 |
| (Drive) `Einheiten 3+1 und FUNiño`, `Einheit L5-6`, `Einheit L6-5`, `Einheit L4-8` | Die Einheiten in Trainersprache nach dem festen Schema |

Die Vorlagen:

| Name | Leitfrage | Ordnung | Hauptteile mit Stationen | Offener Teil |
|---|---|---|---|---|
...
| L4-8 Drei Felder – 3+1 und FUNiño nebeneinander | Frage 4 | 3+1 und FUNiño | **drei** Stationen in **drei** Hauptteilen: 2 gegen 1 plus Torwart (Jugendtor) · FUNiño 3 gegen 1 · FUNiño 3 gegen 2 mit Wandspieler | kein offener Hauptteil – dafür Abschlussturnier auf zwei Feldern (16 Min.) |

Aufbau der ersten acht Vorlagen (70 Min. in Blöcken, 5 Min. Abbauen ohne Block wie bei L4-5):
...
L4-8 hat denselben Rahmen, teilt ihn aber anders auf: drei Hauptteile zu 12 Min. mit derselben
Dreier-Stationsliste, danach 16 Min. Abschlussturnier. Erst nach drei Durchgängen hat jede der
drei Gruppen jedes Feld gesehen – deshalb hier kein offener Hauptteil. Summe ebenfalls 70 Min.,
Netto 38.

**Warum die Hauptteile dieselbe Stationsliste tragen:** `tpVersatz` rückt die Gruppen von
Hauptteil zu Hauptteil eine Station weiter. Bei zwei Feldtrainern ist das der Tausch, bei drei das
Weiterrücken im Kreis – jede Gruppe erlebt jede Station, ohne dass jemand „⇄ weiterrücken“
drückt. Das ist die Logik aus v514/v537, nichts Neues.
...
**L4-8 verteilt die Formate auf drei Felder:** Feld 1 am Jugendtor spielt 3+1-Inhalte, Feld 2 und 3
FUNiño-Inhalte. Weil die Gruppen rotieren, läuft jede Station mit 4 **und** mit 5 Kindern – die
Anpassung steht im Block-Label und in der Skalierung. Braucht drei angehakte Feldtrainer; bei
zweien meldet `_evStationenHinweis` korrekt, dass die dritte Station entfällt (so vorgesehen, siehe
Skalierung 8 und 10).

## Kaderstärke: höchstens 14 Kinder

Der Kader der U9 I umfasst maximal 14 Kinder. Die Skalierungszeile der neun Vorlagen trägt
deshalb die Schlüssel **8, 10, 12 und 14** statt bisher 8/12/16. Die Anzeige liest heute eine fest
verdrahtete Liste `["8","12","16"]` (`md-einheit-import.js`, Vorschau im Übernehmen-Fenster und
`_vaSteckbrief`) – ohne Änderung fielen die Schlüssel 10 und 14 stillschweigend aus der Anzeige.
Deshalb Schritt 3 unten: die Schlüssel aus dem Objekt selbst lesen.
...
`bibliothek.json` plus Nachträge): **keine Fehler**. `_evNettoHinweis` meldet für keine der neun
etwas; `_evStationenHinweis(v, 2)` ist bei den acht Zwei-Stationen-Vorlagen leer und meldet bei
L4-8 die entfallende dritte Station, `_evStationenHinweis(v, 3)` ist bei allen leer. Voraussetzung
war die Erweiterung von `EI_ORDNUNGEN` (Schritt 2) – ohne sie weist die Prüfung alle neun ab.
Alle dreizehn Skizzen sind mit `_skz` gerendert und angesehen; keine handgeschriebene SVG, die
Sperrklinke aus v549 bleibt unberührt.
...
2. **Ordnung:** `EI_ORDNUNGEN` in `md-einheit-import.js` um `"3+1"`, `"FUNiño"`,
   `"3+1 gegen FUNiño"` und `"3+1 und FUNiño"` ergänzen, in dieser Reihenfolge hinter
   `"Raute (4 gegen 4)"`. Keine Migration nötig – die Spalte `trainingsvorlagen.ordnung` ist Text
   ohne Check-Constraint (geprüft in `20260914_vorlagen_ordnung.sql`). Den Spaltenkommentar per
   kleiner Migration nachziehen, damit er nicht lügt. Hilfe-Text zur Kachelreihe „Ordnung“ und
   `v550-vorlagen-ordnung.js` anpassen, falls dort die Liste fest steht. Prüfen, ob die Kachelreihe
   vier weitere Werte am Handy ohne Umbruchprobleme trägt (Skill cockpit-ui, mindestens 48 px).
3. **Skalierung datengetrieben:** In `md-einheit-import.js` an beiden Stellen die feste Liste
   `["8","12","16"]` durch die im Objekt vorhandenen Schlüssel ersetzen, numerisch sortiert
   (etwa `Object.keys(sk).filter(k=>/^\d+$/.test(k)).sort((a,b)=>a-b)`). Damit bestimmen die Daten
   die Zeile, und die nächste Änderung der Kaderstärke kostet keinen Code mehr. Bestehende
   Vorlagen mit 8/12/16 zeigen unverändert ihre drei Werte.
4. **Vorlagen:** Die neun Einträge aus `vorlagen-nachtrag.json` und
   `vorlagen-nachtrag-kombi.json` an `uebungen/vorlagen.json` anhängen, `stand` auf
   `2026-09-16-1`.
5. **Einordnung:** Die dreizehn neuen Übungen in `UEBUNG_ART_VORSCHLAG` (data.js) als `"spiel"`
...
6. **v551:** Die neun Namen als Zusatz-Einheiten benennen (`ZUSATZ` um `L4-6`, `L5-4`,
   `L6-3`, `L4-7`, `L5-5`, `L6-4`, `L5-6`, `L6-5`, `L4-8` erweitern). Die Prüfung der zwanzig
...
7. **Neue Prüfdatei** `tests/checks/v568-aufstellungen-3plus1-funino.js` mit den
   Abnahmekriterien unten.
8. Funktionsübersicht (Stand-Zeile), Hilfe zu „Vorlage übernehmen“ (neue Ordnungen, Skalierung),
   dann `node tests/run.js` grün, dann `sw.js` auf v568.
...
5. Keine der neun löst einen Netto-Hinweis aus; alle tragen die Skalierung 8/10/12/14 und eine
   Beobachtungsfrage mit Aufpasser, Flitzer oder Jäger. Die Skalierungszeile zeigt nach Schritt 3
   **alle vier** Werte, in der Vorschau des Übernehmen-Fensters und im Steckbrief der
   Vorlagen-Ansicht.
6. Im Fenster „Vorlage übernehmen“ erscheinen die Kacheln „3+1“, „FUNiño“,
   „3+1 gegen FUNiño“ und „3+1 und FUNiño“, und sie filtern.
...
9. `L4-8` übernehmen bei **drei** Feldtrainern: drei Felder mit drei verschiedenen Übungen, und
   über die drei Hauptteile rückt jede Gruppe einmal im Kreis weiter (Versatz 0, 1, 2). Bei
   **zwei** Feldtrainern erscheint vor dem Übernehmen der Hinweis, dass die dritte Station
   entfällt.
...
- **Gruppengröße je Station:** Heute teilt `tgBilden` die Kinder gleichmäßig auf, und der Versatz
  dreht die Gruppen weiter – jede Station muss deshalb mit jeder Gruppengröße laufen. Gewünscht
  wäre perspektivisch, dass eine Station eine Größe verlangen kann (etwa „Feld 1 braucht 6“) und
  die Gruppenbildung das berücksichtigt. Bitte nur berichten, was das im Datenmodell (Station,
  `trainingsgruppen`) und in der Rotation bedeuten würde – **nicht bauen**.
